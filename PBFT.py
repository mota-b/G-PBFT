...
# format files
PRE_PREPARE_FORMAT_FILE = "messages_formats/preprepare_format.json"
...
# max nodes and clients values
MAX_CLIENTS_NUMBER = ports["clients_max_number"]
MAX_NODES_NUMBER = ports["nodes_max_number"]

# starting ports values
CLIENTS_PORT_OFFSET = ports["clients_starting_port"]
NODES_PORT_OFFSET = ports["nodes_starting_port"]

# nodes and clients ports
nodes_ports = [(NODES_PORT_OFFSET + i) for i in range(0, MAX_NODES_NUMBER)]
...
nodes_list = []
consensus_nodes_ids = [] # ids of nodes participating in the consensus
consensus_nodes_list = [] # list of nodes participating in the consesus
...
clients_requests = {}  # a dictionary where keys are the clients' ids and the value is the timestamp of their last request # Initiate as an empty dictionary
...
async def run_APBFT(nodes_config, view_changes_timeout, checkpoint_frequency ):  # All the nodes_config participate in the consensus
    ...
    global VIEW_CHANGES_TIMEOUT
    VIEW_CHANGES_TIMEOUT = view_changes_timeout
    ...
    # start nodes_config
    global CSC_list
    threading.Thread(target=run_nodes, args=(nodes_config, )).start()

def run_nodes(nodes_config):
    print("\n\t> B.1) starting nodes ")
    ...
    # Starting nodes:
    print("\t\t> Nodes starting...")
    last_waiting_time = 0
    for waiting_time in nodes_config:
        for tuple in nodes_config[waiting_time]:
            for i in range(tuple[1]):
                ...
                if (node_type == "honest_node"):
                    node = HonestNode(node_id=current_node_id)
                elif (node_type == "non_responding_node"):
                    node = NonRespondingNode(node_id=current_node_id)
                elif (node_type == "faulty_primary"):
                    node = FaultyPrimary(node_id=current_node_id)
                elif (node_type == "slow_nodes"):
                    node = SlowNode(node_id=current_node_id)
                elif (node_type == "faulty_node"):
                    node = FaultyNode(node_id=current_node_id)
                elif (node_type == "faulty_replies_node"):
                    node = FaultyRepliesNode(node_id=current_node_id)
                threading.Thread(target=node.receive, args=()).start()
                nodes_list.append(node)
                the_nodes_ids_list.append(current_node_id)
...
def reply_received(request,
    ...
    if processed_requests % 5 == 0:  # We want to stop counting at 100 for example
        print("\t\t  Network validated %d clients_requests within %faulty_nodes_number_permitted seconds" % (
...
def get_consensus_nodes_list():
    return consensus_nodes_list
...
class Node():
    def __init__(self, node_id):
        ...
        self.stable_checkpoint = {"message_type": "CHECKPOINT", "sequence_number": 0,
                                  ...
                                  "node_id": self.node_id}  # The last stable checkpoint
        self.stable_checkpoint_validators = []  # list of nodes_config that voted for the last stable checkpoint
    ...
    def process_received_message(self, received_message, waiting_time):
        ...
        if (message_type == "REQUEST"):
            ...
            if (client_id not in self.last_reply_timestamp or timestamp > self.last_reply_timestamp[
                client_id]):  # The request is only processed if its timestamp is greater than teh timestamp of the last request that the node responded to
                if (self.node_id == self.primary_node_id):
                    client_id = received_message["client_id"]
                    actual_timestamp = received_message["timestamp"]
                    if (client_id in clients_requests):
                        last_timestamp = clients_requests[client_id]
                    else:
                        last_timestamp = 0
            ...
            # Making sure the node inserted in its message log: a pre-prepare for m in view v with sequence number nodes_number
            p = 0
                ...
                if (self.commits[tuple] == (2 * faulty_nodes_number_permitted + 1) and (tuple in self.prepares)):
                    ...
                    if i == 1:
                        ...
                        if (
                            ...
                            checkpoint_message["checkpoint_digest"] = hashlib.sha256(
                                str(checkpoint_content).encode()).hexdigest()
                ...
                if (received_message["sequence_number"] == checkpoint["sequence_number"] and received_message[
                    ...
                    if (node_id not in self.checkpoints[str(checkpoint)]):
                        ...
                        if (len(self.checkpoints[str(checkpoint)]) == (2 * faulty_nodes_number_permitted + 1)):
                            ...
                            # TO DO: Delete checkpoints and messages log <= nodes_number
            ...
            if (new_asked_view % len(
                    consensus_nodes_ids) == self.node_id):  # If the actual node is the primary node for the next view
                ...
                if len(self.received_view_changes[new_asked_view]) == 2 * faulty_nodes_number_permitted:
                    ...
                    if (max_s > min_s):
                        ...
                        for s in range(min_s, max_s):
                            with open(PRE_PREPARE_FORMAT_FILE):
                                preprepare_format = open(PRE_PREPARE_FORMAT_FILE)
            ...
            if len(O) != 0:
                for message in O:
                    if (received_message["request_digest"] != "null"):
                        ...
                        prepare_message = self.broadcast_prepare_message(message, consensus_nodes_ids)
                        self.message_log.append(prepare_message)
            self.view_number = received_message["new_view_number"]
            self.primary_node_id = received_message["new_view_number"] % nodes_number
    ...
    def check(self, received_message, waiting_time):
        ...
        if len(self.accepted_requests_time) != 0 and len(
            ...
            for request in self.accepted_requests_time:
                if self.accepted_requests_time[request] != -1:
                    ...
                    if (actual_time - timer) >= VIEW_CHANGES_TIMEOUT:
        ...
        if i == 1 or len(self.asked_view_change) != 0:  # Only accept checkpoints, view changes and new-view messages
            ...
            if message_type not in ["CHECKPOINT", "VOTE", "VIEW-CHANGE", "NEW-VIEW"]:
                request = received_message["request"]
                if (message_type == "REQUEST" or (received_message[
                    ...
                    if (client_id in self.replies and received_message == self.replies[client_id][0]):
                        ...
                        threading.Thread(target=self.process_received_message,
                                         args=(received_message, waiting_time,)).start()
            else:
                threading.Thread(target=self.process_received_message, args=(received_message, waiting_time,)).start()
    ...
    def broadcast_message(self, nodes_ids_list, message):  # Send to all connected nodes_config # Acts as a socket server
