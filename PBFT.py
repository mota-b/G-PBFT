...
def random_addr(j):
    ...
    addr = ''.join(random.choice(characters) for i in range(j))
    return addr


class Csc:
    def __init__(self, geohash, csc, address):
        self.geohash = geohash
        self.csc = csc
        self.addr = address

    def csc(self, _geohash):
        self.geohash = _geohash
...
def run_APBFT(nodes, proportion, checkpoint_frequency0, clients_ports0,
