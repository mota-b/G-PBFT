...
def run_PBFT(nodes, checkpoint_frequency0, clients_ports0,
             timer_limit_before_view_change0):  # All the nodes participate in the consensus
...
def stop_request_execution(
        request):  # This method tells the nodes that the client received its reply so that they stop processing it
...
class Node():
    ...
    def process_received_message(self, received_message):
        ...
        if (message_type == "REQUEST"):
                ...
                if (self.node_id == self.primary_node_id):
                    ...
                    if ((last_timestamp < actual_timestamp) or (
                        ...
                        self.broadcast_preprepare_message(request_message=received_message,
                                                          nodes_ids_list=the_nodes_ids_list)
            ...
            if (p == 1 and len(self.prepares[tuple]) == (
                    n - f - 1)):  # The 2*f received messages also include the node's own received message
                self.prepared_messages.append(received_message)

                self.broadcast_commit_message(prepare_message=received_message, nodes_ids_list=the_nodes_ids_list,
                ...
                if (self.commits[tuple] == (2 * f + 1) and (tuple in self.prepares)):
                    ...
                    if i == 1:
                        ...
                        if received_message["request"] in self.accepted_requests_time:
                            self.accepted_requests_time.pop(received_message[
                                                                "request"])  # Discard request from -self.accepted_requests_time- so that its timer stops
                        client_id = received_message["client_id"]
            ...
            if (new_asked_view % len(
                    the_nodes_ids_list) == self.node_id):  # If the actual node is the primary node for the next view
    ...
    def receive(self,
                waiting_time=0):  # The waiting_time parameter is for nodes we want to be slow, they will wait for a few seconds before processing a message =0 by default
        while True:
            ...
            message_type = received_message["message_type"]
            if i == 1 or len(
                    self.asked_view_change) != 0:  # Only accept checkpoints, view changes and new-view messages
