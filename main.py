...
nodes[0] = [("faulty_primary", 0), ("honest_node", 4), ("non_responding_node", 0),
            ("slow_nodes", 0)]  # Nodes starting from the beginning
nodes[1] = [("faulty_primary", 0), ("honest_node", 0), ("non_responding_node", 0),
            ("slow_nodes", 0)]  # Nodes starting after 2 seconds

# Running PBFT protocol
run_PBFT(nodes=nodes, checkpoint_frequency0=checkpoint_frequency, clients_ports0=clients_ports,
...
for i in range(requests_number):
    threading.Thread(target=clients_list[i].send_to_primary,
                     args=("I am the client " + str(i), primary_node_id, nodes_ids_list, f)).start()
